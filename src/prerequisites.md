...
* axum and the other dependencies will be installed using the Rust toolchain, you don't have to worry about that.
...
When a web server responds to a request it includes a status code. The status codes include a number and a default name.
For example "200 OK" means success. "404 NOT FOUND" means the requested page does not exist. "500 INTERNAL SERVER ERROR" means the server crashed. (e.g. there was an unhandled exception in the code.)
